...
function bill_ct($listpro)
{
    global $img_path;
    $sumall = 0;
    $i = 0;
    foreach ($listpro as $cart) {
        // Mặc du báo lỗi không hiểu biến $img_path nhưng vẫn cho ra kết quả đúng là đường dẫn ảnh
        $img = $img_path . $cảt['image'];
        $sumall+= $cảt['thanhtien'];
...
function insert_bill($name,$email,$address,$phone,$pttt,$date_buy,$tongdonhang){
    $sql="INSERT INTO bill(`bill_name`, `bill_address`, `bill_phone`, `bill_email`, `bill_pttt`, `date_buy`, `sum`) value('$name','$address','$phone','$email','$pttt','$date_buy','$tongdonhang')";
    return pdo_execute_lastInsertId($sql);
}
function insert_cart($id_user,$id_hh,$image,$name,$price,$soluong,$thanhtien,$id_bill){
    $sql="insert into `cart`(`id_user`, `id_hh`, `name`, `image`, `price`, `soluong`, `thanhtien`, `id_bill`) value('$id_user','$id_hh','$name','$image','$price','$soluong','$thanhtien','$id_bill')";
    pdo_execute($sql);
}
