    ...
    switch ($act) {
        ...
        case 'billcomfirm':
            if (isset($_POST['dongy'])) {
                ...
                $address=$_POST['address'];
                $phone=$_POST['phone'];
                $pttt=$_POST['bordered-radio'];
                $date_buy=date('h:i:sa d/m/Y');
                $tongdonhang=tongdonhang();
                $id_bill=insert_bill($name,$email,$address,$phone,$pttt,$date_buy,$tongdonhang);

                //insert into bảng cart: lấy dữ liệu từ session mycart
                foreach ($_SESSION['mycart'] as $show) {
                    insert_cart($_SESSION['user']['id'],$show[0],$show[1],$show[2],$show[3],$show[4],$show[5],$id_bill);
                }

                $_SESSION['mycart']=[];
            }
            // $bill=loadone_bill($bill);
            // $billct=loadone_cart($ibillct);
