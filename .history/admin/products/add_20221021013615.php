...
if (isset($_POST['themmoi'])) {
    ...
    if ($sale < 0) {
        $sale_err = "Giá sale phải là số dương";
    }elseif(!is_numeric($sale)){
        $sale_err = "hãy nhập số";
    }
     else {
        $sale_err = "";
    }
    ...
    if ($view < 0) {
        $view_err = "Lượt xem phải là số dương";
    } else {
        $view_err = "";
    }
}
        ...
        $().ready(function() {
            $("#checkproduct").validate({
                ...
                messages: {
                    ...
                    "image": {
                        equalTo: "Bắt buộc chọn hình ảnh",
                        minlength: "Hãy nhập ít nhất 8 ký tự"
                    },
                    "detail": {
                        equalTo: "Bắt buộc nhập mô tả chi tiết",
                        // minlength: "Hãy nhập ít nhất 8 ký tự"
                    },
                    "view": {
                        equalTo: "Bắt buộc nhập số lượt xem",
                        // minlength: "Hãy nhập ít nhất 8 ký tự"
                    }
                }
                ...
                <form onsubmit="return checkSelect()" action="index.php?act=addhh" method="post" id="checkproduct" enctype="multipart/form-data">
                    <div class="grid grid-cols-3 gap-8">
                        ...
                        <div>
                            ...
                            <input class="border w-full p-2 rounded-[4px]" type="text" name="price" id="price" value="<?= isset($price) ? $price : '' ?>">
                            <?php
                            if (isset($price_err)) : ?> <br>
                                <p class="flex justify-end text-red-500"><?= $price_err ?></p>
                            <?php endif ?>
                        </div>
                        <div>
                            <p class="font-bold mt-3 mb-2">Giảm giá</p>
                            <input class="border w-full p-2 rounded-[4px]" type="text" name="sale" id="sale" value="<?= isset($price) ? $price : '' ?>">
                            <?php
                            if (isset($sale_err)) : ?> <br>
                                <p class="flex justify-end text-red-500"><?= $sale_err ?></p>
                            <?php endif ?>
                        </div>
                        <div>
